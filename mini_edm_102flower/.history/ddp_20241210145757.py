...
class RegressionModel(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(1, 64)  # 输入是 1D 数据，输出是 64 维
        self.fc2 = nn.Linear(64, 32) # 第二层 32 维
        self.fc3 = nn.Linear(32, 1)  # 最后一层输出 1D 数据，即回归目标
...
def get_dataset():
    ...
    dataset = QuadraticDataset(start=-10, end=10, num_samples=10000)
    train_sampler = torch.utils.data.distributed.DistributedSampler(dataset)
    trainloader = DataLoader(dataset, batch_size=8, num_workers=2, sampler=train_sampler)
    return trainloader
